# Point production login at SMPL Demo Co (default demo for outreach).
#
# Session-sync picks the active org with the oldest joined_at. This script:
#   1. Ensures active admin membership on SMPL Demo Co
#   2. Sets that membership joined_at earliest; bumps other orgs later
#
# Usage:
#   .\scripts\set-prod-demo-workspace.ps1
#   .\scripts\set-prod-demo-workspace.ps1 -Email mattjustice@smpl-ai.com
...
Write-Host ""
Write-Host "=== Production default workspace -> SMPL Demo Co ===" -ForegroundColor Cyan
Write-Host "DB host: $safeHost" -ForegroundColor DarkGray
Write-Host "Email:   $email" -ForegroundColor DarkGray
Write-Host ""
Write-Host "Must match Vercel Production AUTH_DATABASE_URL and Railway prod DATABASE_URL host." -ForegroundColor Yellow
Write-Host ""
...
with engine.begin() as conn:
    ...
    if user is None:
        print(f"ERROR: No users row for {email}. Sign in on prod once, then re-run.", file=sys.stderr)
        sys.exit(1)
    user_id = user[0]
...
print("")
print("Active workspaces (first = default on next login):")
...
print("Sign out at https://smpl-financial-intelligence.vercel.app/login")
print("Request a NEW magic link (incognito). Do not reuse an old link.")
"@
