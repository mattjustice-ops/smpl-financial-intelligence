...
def _response(waterfall_name: str, organization_id: uuid.UUID, db: Session, **params) -> WaterfallResponse:
...
def _attribution(waterfall_name: str, organization_id: uuid.UUID, db: Session, **params) -> list[WaterfallAttributionRow]:
    return _response(waterfall_name, organization_id, db, **params).attribution
...
def arr(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> WaterfallResponse:
    return _response("arr", organization_id, db, **params)


@waterfall_router.get("/arr/attribution", response_model=list[WaterfallAttributionRow])
def arr_attr(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> list[WaterfallAttributionRow]:
    return _attribution("arr", organization_id, db, **params)


@waterfall_router.get("/pipeline", response_model=WaterfallResponse)
def pipeline(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> WaterfallResponse:
    return _response("pipeline", organization_id, db, **params)


@waterfall_router.get("/pipeline/attribution", response_model=list[WaterfallAttributionRow])
def pipeline_attr(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> list[WaterfallAttributionRow]:
    return _attribution("pipeline", organization_id, db, **params)
...
def deferred_revenue(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> WaterfallResponse:
    try:
        result = _response("deferred_revenue", organization_id, db, **params)
        # Fail here with a readable message if the response cannot serialize.
...
def deferred_revenue_attr(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> list[WaterfallAttributionRow]:
    return _attribution("deferred_revenue", organization_id, db, **params)


@waterfall_router.get("/cash-flow", response_model=WaterfallResponse)
def cash_flow(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> WaterfallResponse:
    return _response("cash_flow", organization_id, db, **params)


@waterfall_router.get("/cash-flow/attribution", response_model=list[WaterfallAttributionRow])
def cash_flow_attr(organization_id: uuid.UUID = Query(...), params: dict = Depends(route_params), db: Session = Depends(get_db)) -> list[WaterfallAttributionRow]:
    return _attribution("cash_flow", organization_id, db, **params)
