...
**Collection status:** **not complete** — Railway Type II **received** (review pending); Vercel access **requested** (waiting); other P0 still open. Public Stripe SOC 3 summary only (not Type II). Readiness only — not SOC 2 certified.
...
3. **Neon** — **Next:** [trust.neon.com](https://trust.neon.com/) → sign in as **paid** customer → request **SOC 2** (and ISO if listed) → wait ~2 business days if gated.
