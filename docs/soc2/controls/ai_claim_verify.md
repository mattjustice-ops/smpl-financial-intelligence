...
| MD&A Prompt 5 deck | **Widened evidence package** (actuals ≤ close, forecast after close, pipeline/deals, cash/ARR bridges, variance drivers) + `_sources` (`series_kind` where tagged) + attribution (incl. forecast/pipeline forward keys) in prompt; post-LLM verify of **$ / % / Nx in PPTX string literals**; numeric + citation **soft-strip** (export continues); attribution surgical strip; prefer export even when fully wiped. Prompt: prefer package numbers/drivers, label actual vs forecast vs pipeline, rich narrative encouraged — not sparse | `prompt5_deck.py`, `board_platform_metrics.build_evidence_package_from_deck_payload` |
...
| Prompt 5 deck generation | **Live (soft-strip + export)** | Widened evidence (actual/forecast/pipeline/bridges) so rich board story can use package context without false shutdowns; invent still soft-strips; citation soft-strip; attribution surgical strip; **export continues**. Chart array / layout coords not scanned. Export-time warehouse / client A–F validation unchanged (advisory). |
