...
| MD&A Prompt 5 deck | **Widened evidence package** (actuals ≤ close, forecast after close, pipeline/deals, cash/ARR bridges, variance drivers) + `_sources` (`series_kind` where tagged) + attribution (incl. forecast/pipeline forward keys) + **freeze prose** in fresh **and adapt-first** prompts; post-LLM verify of **$ / % / Nx in PPTX string literals**; numeric + attribution **soft-strip to `—`** (never multi-sentence don't-know essays in PPTX cells); citation **warn-only** (do not wipe uncited board cells); prefer export even when fully wiped. Prompt: cite in takeaways, not inside KPI/table cells. **Narrative depth:** Key Takeaways / risks / board actions require 3–5 insight bullets (driver + variance, retention/pipeline quality, root cause, Actual vs Forecast vs Pipeline, recommended board action) — regenerate/Copilot shape; KPI/table cells stay numbers/`—` only | `prompt5_deck.py`, `prompt5_narrative.py`, `prompt5_adapt.py`, `board_platform_metrics.build_evidence_package_from_deck_payload` |
...
| MD&A Prompt 2 | **Live (harder / strict)** | Matrix mismatch → hard block; post-LLM numeric + attribution + citation strip; **narrative verify** (filler/placeholder/direction/scenario) soft-strip; fully unverifiable variance sheet → hard block |
| Prompt 5 deck generation | **Live (soft-strip + export)** | Widened evidence (actual/forecast/pipeline/bridges) + freeze in fresh **and adapt**; takeaway prompts require 3–5 insight bullets; invent / bad attribution soft-strip to `—`; citation **warn-only**; **export continues**. **Narrative verify** soft-strips filler/placeholder on literals. **Post-render fidelity** re-checks finished PPTX metric cells + ARR/revenue/cash anchors + narrative (soft-warn). Chart array / layout coords not scanned. |
...
| `backend/tests/test_narrative_and_post_render_verify.py` | Filler / placeholder / direction / scenario narrative checks; nested MD&A strip; PPTX text extract; post-render anchor pass/fail; narrative flagged on rendered shapes |
