...
| `[~]` | Subprocessor inventory + vendor reports collected | Inventory **locked** 2026-07-28; five P0 Type II **reviewed** 2026-08-03; **Vercel** open; GitHub deferred — [02_subprocessors.md](./02_subprocessors.md) + [TRACKER.md](./evidence/vendor-soc/TRACKER.md) |
...
4. **Live on** Prompt 5 deck — evidence + `_sources` in prompt; post-LLM hard-block on invented $/% in PPTX string literals; **citation soft-strip / hard-block-when-fully-wiped**
5. **Live on** board slide regenerate — per-bullet soft strip / don't-know (numeric + attribution + **citation**)
...
8. **Citation verify:** post-LLM material money/%/Nx must cite `_sources` token on commentary generate, MD&A Prompt 2, Copilot, **Prompt 5 PPTX literals**, **board regenerate** — [controls/ai_claim_verify.md](./controls/ai_claim_verify.md)
...
10. **DOM `data-source` overlay** + audit hotkey on Board/FE KPIs; **client `runTieOut` Rule Sets A–F** (+ client HTML report) gates live MD&A export + FINAL forecast promote — [controls/fe_board_single_source.md](./controls/fe_board_single_source.md)
...
## What we checked off 2026-08-03 — Vendor SOC P0 Type II reviews (Matt skim)

**Not SOC 2 certified.** Skim review ≠ CPA Type I. Scoreboard stays **[~]** until Vercel is reviewed (or honestly blocked). No PDF findings invented.

1. Matt attested skim reviews for **Railway, Neon, Stripe, Anthropic, Resend** → tracker `reviewed` (2026-08-03)
2. Flipped “Vendor report collected?” in [02_subprocessors.md](./02_subprocessors.md) for those five only
3. **Vercel** remains `requested` (waiting SafeBase / Trust Center approval) — Type II **not** received
4. **GitHub P1 deferred** until Type I audit engagement (Matt decision)
5. Updated [TRACKER.md](./evidence/vendor-soc/TRACKER.md), [WORKING_PACK_2026-08.md](./evidence/vendor-soc/WORKING_PACK_2026-08.md), scoreboard + `/compliance` mirror
