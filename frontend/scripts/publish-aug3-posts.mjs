/**
 * Upsert the Aug 3 blog posts (ARR governance + revenue model shapes finance).
 *
 * Usage (from frontend/):
 *   node scripts/publish-aug3-posts.mjs
 *   # or: npm run publish:aug3-posts
 */
...
import { aug3Posts } from "../sanity/seed/aug3-posts.mjs";
...
async function main() {
  for (const doc of aug3Posts) {
    if (doc._id.includes(".")) {
      throw new Error(`Seed doc ${doc._id} uses '.' — use hyphens.`);
    }
  }

  const slugs = aug3Posts.map((p) => p.slug.current);
  ...
  console.log(`Published ${aug3Posts.length} posts → ${projectId}/${dataset}`);
  for (const p of aug3Posts) {
